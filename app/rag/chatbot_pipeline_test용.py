...
#                     "k": 50,
#                     "num_candidates": 100
#                 }, _source=["card_name", "card_id", "benefits.summary", "benefits.category"])
#                 vec_hits = vec_res["hits"]["hits"]
#             except Exception as e:
#                 print(f"⚠️ Vector Search Error: {e}")

#         # --- 2. Keyword Search (Nested Sum + Boosts) ---
#         # 💡 KEY CHANGE: score_mode='sum'과 Boost 적용
#         # should 내부에 match 쿼리를 넣고 boost를 차등 적용합니다.
...
#                 "nested": {
#                     "path": "benefits",
#                     "score_mode": "sum",  # 💡 일치하는 혜택들의 점수를 합산 (Summed Score)
#                     "query": {
#                         "bool": {
...
#                         }
#                     },
#                     "inner_hits": {  # 매칭된 혜택 하이라이팅을 위해 가져옴
#                         "_source": ["benefits.summary", "benefits.category", "benefits.value"],
#                         "size": 3
#                     }
#                 }
#             }, size=50, _source=["card_name", "card_id", "benefits.summary", "benefits.category"])
...
#             item["is_must"] = group.get("is_must", False)  # 👈 이 부분 추가 필수!
...
#                     "total_score": 0.0,
#                     "matched_reasons": [],
#                     "matched_summaries": set(), # 🌟 [변경] 여러 혜택을 담을 집합(Set)
...
#             # 🌟 [추가] Inner Hits(검색된 혜택)는 무조건 결과 리스트에 추가
...
#             for group in groups:
#                 g_name = group["name"]
#                 if g_name in card["matched_group_names"]: continue # 이미 찾은 그룹은 패스
...
#             # (2) 🌟 [핵심] 최종 매칭된 모든 그룹의 키워드로 혜택을 전수 조사하여 수집
#             #     (Inner Hit에 없던 Cross-Check 발견 혜택 등을 모두 긁어모음)
#             active_keywords = []
#             for group in groups:
