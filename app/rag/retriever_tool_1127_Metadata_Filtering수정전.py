...
def calculate_benefit_score(doc, user_brands: List[str], user_categories: List[str]) -> float:
    try:
        ...
        # 1. 단위 보정 (Unit Normalization)
        # % 단위는 100배를 곱하여 KRW와 형평성을 맞춤
        if unit == "%":
            score = base * 100
        else:
            score = base 
        ...
        if user_categories:
            ...
            for cat in user_categories:
                # 대소문자 무시 비교 등 유연하게 처리
                if cat in doc_category or doc_category in cat:
                    score *= 1.5
                    match_found = True
                    break
            
            # 카테고리 키워드가 텍스트 내에 포함될 경우 (보조)
            if not match_found and any(cat in text_all for cat in user_categories):
                score *= 1.2
